...
run_task() {
  ...
  if result_complete "$task_result" "$dataset" "$condition" "$outer_seed"; then
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/tivit_outer_seed_mpl_gpu_${PHYSICAL_GPU}}"
export HF_HOME="${HF_HOME:-/tmp/tivit_outer_seed_hf}"
