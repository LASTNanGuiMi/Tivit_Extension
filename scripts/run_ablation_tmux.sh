...
summary_command="cd '$PROJECT_DIR'; python3 scripts/aggregate_ablation.py --result-root '$RESULT_ROOT' --seeds $SEEDS --repeats '$REPEATS' --datasets $DATASET_GROUPS --status-dir '$STATUS_DIR' --workers '$NUM_WORKERS' --output '$SUMMARY_FILE'"
